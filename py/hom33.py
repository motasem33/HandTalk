class Pruduct:
    ...
    def show_info(self):
        print("product", self.name, "price", self.price, "stock", self.stock)
    ...
    def purchase(self, pruduct, quantity):
        ...
        if self.balance >= discount_cost:
            if pruduct.buy(quantity):
                ...
                print("premium", self.name, "bought", quantity, pruduct.name,
                      "with", self.discount * 100, "% off! balance:", self.balance)
        else:
            print(self.name, "has insufficient funds!")
...
laptop.show_info()
print("testing premium customer")
mouse.show_info()
